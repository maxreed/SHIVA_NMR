...
def compute_dssp_stats_framewise(pdb_path: str, xtc_path: str) -> pd.DataFrame:
    """
    Load a heavy-atom trajectory directly and compute per-frame φ/ψ and full DSSP one-hot.

    - pdb_path: path to reference PDB (heavy atoms only)
    - xtc_path: path to trajectory XTC (nm units)
    ...
    Outputs a DataFrame with columns:
      ...
      and one-hot columns ss_prev_<code>, ss_<code>, ss_next_<code>
      for all DSSP codes: H, B, E, G, I, T, S, C (coil).

    Ensures consistent columns even if some SS codes aren't observed.
    """
    # Load trajectory and convert coords to Ångstroms for DSSP
